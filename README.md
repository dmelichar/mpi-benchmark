...
- **increasing**: Generates an increasing sequence of block sizes based on a specified average block size (avg) and number of processors (nproc). It follows that for each message $$m_i = \left \lfloor \frac{2\operatorname{avg}*(i+1)}{\operatorname{nproc}-1} \right \rfloor$$ with $i = 0,1,...,nproc-1$ 
- **decreasing**: Generates a decreasing sequence of block sizes based on a specified average block size (avg) and number of processors (nproc). It follows that for each message $$m_i = \left \lfloor \frac{2\operatorname{avg}*(\operatorname{nproc}-1)}{\operatorname{nproc}} \right \rfloor+1$$ with $i = 0, 1, ..., nproc-1$
