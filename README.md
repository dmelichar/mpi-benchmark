...
We provide the following distributions with $i = 0, 1, ..., nrpoc-1$:
...
``` math
m_i = \left \lfloor \frac{2\mathop{avg}*(i+1)}{\mathop{nproc}-1} \right \rfloor
```  
...
``` math
m_i = \left \lfloor \frac{2\mathop{avg}*(\mathop{nproc}-1)}{\mathop{nproc}} \right \rfloor+1
```
...
``` math
m_i = \frac{\mathop{avg}}{2} + x$$ and $x \in [1, \mathop{avg}]
``` 
...
``` math
m_i = \begin{cases}\mathop{avg} + \frac{\mathop{avg}}{2} && \text{if } i \text{ is even} \\ \mathop{avg} - \frac{\mathop{avg}}{2} && \text{if } i \text{ is odd}\end{cases} 
``` 
...
``` math
m_i = \begin{cases}\mathop{avg} && \text{if } i = 0 \text{ or } i = nproc-1 \\ 0 && \text{otherwise}\end{cases}
``` 
