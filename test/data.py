...
def equal(nproc: int, val: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.full(size, val, dtype=np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-equal.csv"
        values = values.reshape(1, -1) if not m2m else values
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def normal(nproc: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.abs(np.random.normal(10, 15, size)).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-normal.csv"
        values = values.reshape(1, -1) if not m2m else values
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def exponential(nproc: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.random.exponential(50, size).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-exponential.csv"
        values = values.reshape(1, -1) if not m2m else values
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def increasing(nproc: int, avg: int, seed: int = 42):
        np.random.seed(seed)
        values = np.floor((2 * avg * (np.arange(nproc) + 1)) / nproc).astype(np.uintp)
        f = f"{nproc}-increasing.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def decreasing(nproc: int, avg: int, seed: int = 42):
        np.random.seed(seed)
        values = (np.floor((2 * avg * (nproc - np.arange(nproc))) / nproc) + 1).astype(np.uintp)
        f = f"{nproc}-decreasing.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def zipfian(nproc: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.random.zipf(2, size).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-zipfian.csv"
        values = values.reshape(1, -1) if not m2m else values
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def uniform(nproc: int, avg: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.random.randint(1, 2 * avg, size=size).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-uniform.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def bucket(nproc: int, avg: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = (avg // 2) + np.random.randint(1, avg, size=size).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-bucket.csv"
        values = values.reshape(1, -1) if not m2m else values
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def spikes(nproc: int, avg: int, rho: float = 2, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        size = nproc if not m2m else (nproc, nproc)
        values = np.random.choice([rho * avg, 1], size=size, p=[1 / rho, 1 - 1 / rho]).astype(np.uintp)
        f = f"{nproc}{'-m2m' if m2m else ''}-spikes.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def alternating(nproc: int, avg: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        values = np.array([avg + avg // 2 if i % 2 == 0 else avg - avg // 2 for i in range(nproc)]).astype(np.uintp)
        if m2m:
                values = np.tile(values, (nproc, 1))
        f = f"{nproc}{'-m2m' if m2m else ''}-alternating.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f


def two_blocks(nproc: int, avg: int, m2m: bool = False, seed: int = 42):
        np.random.seed(seed)
        values = np.zeros(nproc, dtype=np.uintp)
        values[0] = avg
        values[-1] = avg
        if m2m:
                values = values.reshape((nproc, nproc))
        f = f"{nproc}{'-m2m' if m2m else ''}-two-blocks.csv"
        np.savetxt(f, values, delimiter=",", fmt="%d")
        return values, f
...
if __name__ == "__main__":
        ...
        if args.command == 'equal':
                equal(args.nproc, args.val, args.m2m, args.seed)
        elif args.command == 'normal':
                normal(args.nproc, args.m2m, args.seed)
        elif args.command == 'exponential':
                exponential(args.nproc, args.m2m, args.seed)
        elif args.command == 'increasing':
                increasing(args.nproc, args.avg, args.seed)
        elif args.command == 'decreasing':
                decreasing(args.nproc, args.avg, args.seed)
        elif args.command == 'zipfian':
                zipfian(args.nproc, args.m2m, args.seed)
        elif args.command == 'uniform':
                uniform(args.nproc, args.avg, args.m2m, args.seed)
        elif args.command == 'bucket':
                bucket(args.nproc, args.avg, args.m2m, args.seed)
        elif args.command == 'spikes':
                spikes(args.nproc, args.avg, args.rho, args.m2m, args.seed)
        elif args.command == 'alternating':
                alternating(args.nproc, args.avg, args.m2m, args.seed)
        elif args.command == "two_blocks":
                two_blocks(args.nproc, args.avg, args.m2m, args.seed)
