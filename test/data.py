...
def dgp(nproc, m2m, dist, filename, seed):
    ...
    if dist == "normal":
        result = np.abs(np.random.normal(10, 15, size)).astype(np.uintp)
    elif dist == "exponential":
        result = np.random.exponential(50, size).astype(np.uintp)
    elif dist == "increasing":
        if not m2m:
            result = np.arange(size, dtype=np.uintp)
        else:
            result = np.arange(np.prod(size), dtype=np.uintp).reshape(size)
    elif dist == "zipfian":
        result = np.random.zipf(2, size).astype(np.uintp)
    elif dist == "equal":
        result = np.full(size, 10, dtype=np.uintp)
    else:
    ...
    result = result.reshape(1, -1) if not m2m else result
    np.savetxt(f, result, delimiter=",", fmt="%d")
