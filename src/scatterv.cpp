    ...
    void run (size_t min_msg_size, size_t max_msg_size, double max_seconds = 1, bool verbose = false) { 
        ...
        for (size_t size = min_msg_size; size <= max_msg_size; size *= 2) {
            ...
            while (true) {    
                ...
                // sbuffer, sendcounts, displ, MPI_DOUBLE, rbuffer, size, MPI_DOUBLE, 0, MPI_COMM_WORLD 
                //MPI_Scatterv(sbuffer.data(),, MPI_DOUBLE, rbuffer.data(), size, MPI_DOUBLE, MPI_COMM_WORLD);
...
int main(int argc, char *argv[]) {
    ...
    try { 
        ...
        for (auto msg_size : msg_sizes) {
            ...
            benchmark.save_latencies("scatterv-latencies.csv");
        }
